...
function updateSubtotal(id, harga, qty) {
  let subtotal = harga * qty;
  let el = document.getElementById('subtotal-' + id);
...
document.querySelectorAll('.plus').forEach(btn => {
  btn.addEventListener('click', function () {
    ...
    updateSubtotal(id, harga, input.value, 'plus');
    updateTotal();
    updateBackend(id, input.value);
  });
});
...
document.querySelectorAll('.minus').forEach(btn => {
  btn.addEventListener('click', function () {
    ...
    if (input.value > 1) {
      ...
      updateSubtotal(id, harga, input.value, 'minus');
      updateTotal();
      updateBackend(id, input.value);
    }
  });
